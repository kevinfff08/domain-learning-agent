...
class AdaptiveController:
    ...
    def determine_level(
        ...
        Uses BKT mastery estimate when available, falls back to quiz score.
        """
        # Prefer BKT mastery if available
        ...
        if concept.adaptive_level >= AdaptiveLevel.ALTERNATIVE_EXPLANATION and mastery < 0.3:
  ...
  {{
    ...
    "tags": ["prerequisite", "math"|"programming"|"domain"]
        ...
        try:
            ...
            prereqs_data = repair_json_array(response)
...
[
  {{
    ...
    "tags": ["sub_concept"]
        ...
        try:
            response = self.llm.generate_json(prompt)
            subs_data = repair_json_array(response)
        except (ValueError, Exception):
            subs_data = [
